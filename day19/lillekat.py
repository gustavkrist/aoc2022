...
def parse_input(s: str) -> list[Recipe]:
    blueprints = []
...
def part1(blueprints: list[Recipe]) -> int:
    result = 0
    for i, recipe in enumerate(blueprints, start=1):
        ...
        best = 0
        best_at_time = {i: 0 for i in range(24)}
...
def part2(blueprints: list[Recipe]) -> int:
    result = 1
    for recipe in blueprints[:3]:
        can_build.cache_clear()
        limit_minerals.cache_clear()
        generate_states.cache_clear()
        limits = {"ore": 0, "clay": 0, "obsidian": 0, "geode": 10000000}
...
with open(0) as f, support.timing():
    blueprints = parse_input(f.read().rstrip())
    with support.timing():
